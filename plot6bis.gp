...
a=2
b=1
fit f(x) "./moyennegyrationN.txt" u (log(($1-1)/2)):(log($2)) via a,b
g(x)=exp(b)*x**a
#h(x)= (exp(b)/x)/3
set terminal postscript eps color "Times-Roman" 24
set output 'gyrationradius.eps'
...
!epstopdf --outfile=gyrationradius.pdf gyrationradius.eps 
quit
